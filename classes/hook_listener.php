...
class hook_listener {
     ...
     *
     * @param \core_enrol\hook\after_user_enrolled $hook Hook
     */
    public static function send_course_welcome_message(after_user_enrolled $hook): void {
        global $CFG, $DB;
        $instance = $hook->get_enrolinstance();
        if ($instance->enrol == 'coursecompleted' && enrol_is_enabled('coursecompleted')) {
            // Send welcome message.
            if (
                $instance->customint2 != ENROL_DO_NOT_SEND_EMAIL &&
            ...
            if ($instance->customint5 > 0 && $context = context_course::instance($instance->customint1, IGNORE_MISSING)) {
